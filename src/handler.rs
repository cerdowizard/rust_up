...
pub async fn create_note_handler(
    ...
    let query_result = sqlx::query_as!(
        ...
        payload.category.to_owned().unwrap_or("".to_string())
    )
    .fetch_one(&data.db)
    .await;
match query_result {
...
pub async fn get_single_note_handler(
...
pub async fn delete_note_handler(
    ...
    let query_result = sqlx::query!("DELETE FROM notes WHERE id = $1", id)
        .execute(&data.db)
        .await
        .unwrap()
        .rows_affected();
...
pub async fn edit_note_handler(
    ...
    if query_result.is_err() {
        let error_response = serde_json::json!({
            "status": "fail",
            "message": format!("Note with ID: {}", id),
        });
        return Err((StatusCode::NOT_FOUND, Json(error_response)));
    }
    ...
    let query_result = sqlx::query_as!(
        ...
        payload.title.to_owned().unwrap_or(note.title),
        payload.content.to_owned().unwrap_or(note.content),
        payload.category.to_owned().unwrap_or(note.category.unwrap()),
        payload.published.unwrap_or(note.published.unwrap()),
        now,
        id
    )
    .fetch_one(&data.db)
